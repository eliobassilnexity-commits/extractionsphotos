...
if (st.session_state.uploaded_excel and st.session_state.uploaded_pdf 
        ...
        for page_num in range(pages_to_extract):
            ...
            if page_num != photo_last_page_idx:
                ...
                if nb_photos_page not in (3, 6):
                    ...
                    st.error(
                        f"❌ Incohérence détectée dans le nombre de photos par désordre à la page {page_num + 1}. Corriges celà directement sur le projet Archipad, exporte à nouveau les rapports PDF et Excel, puis reviens ici pour le traitement."
                    )
                    if os.path.exists(output_folder):
                        shutil.rmtree(output_folder)
                    st.stop()
    ...
    if nb_photos_sans_derniere_page == nb_lignes_plan:
        ...
        if nb_img_restantes == nb_lignes_plan:
            ...
            st.error(f"❌ Incohérence détectée dans le nombre de photos par désordre. Corriges celà directement sur le projet Archipad, exporte à nouveau les rapports PDF et Excel, puis reviens ici pour le traitement.")
            shutil.rmtree(output_folder)
            st.stop()
